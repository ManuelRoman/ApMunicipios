...
/**
 * Cambia el nombre del municipio seleccionado
 * Participa en la acción 4
 */
public class CambiarNomMun implements Accion{
	...
	public boolean ejecutar(HttpServletRequest request, HttpServletResponse response)
		...
		String nombreAnterior = request.getParameter("nomAnt");
		String nombreNuevo = request.getParameter("nomNuevoMun");
		Municipio municipioExistente = null;
		System.out.println("Cambiar " + nombreAnterior + " por " + nombreNuevo);
		String json = null;
		this.error = null;
		...
		try {
			...
			municipioExistente = daoConsultas.getMunicipio(nombreAnterior);
			daoInsercion.cambiarNomMun(municipioExistente, nombreNuevo);
		...
		json = new Gson().toJson(nombreAnterior);
