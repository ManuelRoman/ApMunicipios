...
/**
 * Ofrece el autocompletado del nombre del municipio de la provincia escogida
 * Participa en la acción 2
 */
public class AutoMunProv implements Accion{
	/**
	...
	public boolean ejecutar(HttpServletRequest request, HttpServletResponse response)
		...
		// Si la llamada viene del objeto XmlHttpRequest, hay que cambiar la codificación del request
		if(request.getHeader("x-requested-with")!=null && request.getHeader("x-requested-with").equals("XMLHttpRequest")) {
		    request.setCharacterEncoding("UTF-8");
		}
		String idProvincia = request.getParameter("selProvincia");
		String datos = request.getParameter("nomMun");
		String respuesta = null;
		System.out.println("Solicitado autocompletar nombre de municipio de la provincia: " + idProvincia);
		...
		try {
			...
			respuesta = daoConsultas.getNombreMun(Short.parseShort(idProvincia), datos);
