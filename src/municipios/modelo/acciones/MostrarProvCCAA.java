...
/**
 * Devuleve una lista de las provincias que tiene una comunidad autónoma
 * Participa en la acción 1 y 3
 */
public class MostrarProvCCAA implements Accion{
	...
	public boolean ejecutar(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("Comunidad seleccionada: "+idComunidad);
		String json = null;
		HashMap<Integer, String> listaProvincias = new HashMap<Integer, String>();
		...
		for ( Iterator iterador = comunidad.getProvincias().listIterator(); iterador.hasNext(); ) {
		      Provincia provincia = (Provincia) iterador.next();
		      listaProvincias.put((int) provincia.getIdProvincia(), provincia.getProvincia());
		}
		modelo.setContentType("application/json;charset=UTF-8");
		json = new Gson().toJson(listaProvincias);
